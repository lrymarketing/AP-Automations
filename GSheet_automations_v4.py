...
# Constants for better control and readability
MAX_RETRIES = 3
...
def make_request_with_retries(url, method='get', headers=None, params=None, json_body=None, max_retries=MAX_RETRIES, timeout=10):
    """Makes HTTP request with retries on failure."""
    retries = 0
    while retries < max_retries:
        try:
            ...
            log_message(f"Request failed, retrying... ({retries+1}/{max_retries}) Error: {e}")
            retries += 1
            time.sleep(SLEEP_INTERVAL * retries)  # Exponential backoff
    raise requests.exceptions.RequestException(f"Failed after {max_retries} retries.")

# Function to safely execute a Google Sheets API request with retries
def execute_google_sheets_api_request(request, max_retries=MAX_RETRIES):
    """Executes a Google Sheets API request with retries on failure."""
    for attempt in range(max_retries):
        try:
            return request.execute()
        except HttpError as e:
            if e.resp.status == 500:  # Internal Server Error
                log_message(f"HttpError 500 encountered on attempt {attempt+1}. Retrying after delay...")
                time.sleep((2 ** attempt) + random.random())  # Exponential backoff
            else:
                raise  # Re-raise the exception if it's not an HTTP 500 error
        except Exception as e:
            log_message(f"Non-HttpError occurred: {e}, Retrying after delay...")
            time.sleep((2 ** attempt) + random.random())  # Exponential backoff
    raise Exception("Google Sheets API request failed after retries.")
            
def get_sheet_size(service, spreadsheet_id, sheet_name):
    # Fetches the sheet metadata with retries
    sheet_metadata_request = service.spreadsheets().get(spreadsheetId=spreadsheet_id)
    sheet_metadata = execute_google_sheets_api_request(sheet_metadata_request)
    sheets = sheet_metadata.get('sheets', [])

    # Finds the specific sheet and retrieves its size
    for sheet in sheets:
    ...
    return None, None  # In case the sheet is not found
...
class GoogleSheetsUpdater:
    ...
    def check_api_availability(self):
        ...
        try:
            response = make_request_with_retries(api_base_url)
            log_message(f"API Status Check: URL: {api_base_url}, Response Code: {response.status_code}, Response Body: {response.text}")
            if response.status_code == 200 and response.json().get('code') == 0:
                log_message("Adspower API is available.")
                return True
            else:
                log_message(f"Adspower API is not available: {response.text}")
                return False
        except Exception as e:
            log_message(f"Failed to check API availability: {e}")
            return False
    ...
    def run(self):
        ...
        for sheet_name in sheet_names:
            ...
            if total_rows is None or total_columns is None:
                print(f"Unable to find sheet: {sheet_name}")
                continue

            # Delay after fetching the sheet size
            time.sleep(1)
            ...
            for i in range(4, total_rows + 1):
                try:
                    ...
                    address = str(row_values[3]) if len(row_values) > 3 else None # Column D
                    print(f"Address: {address}, Type: {type(address)}")  # Debugging print statement
                    ...
                    print(f"Debug before update_adspower_remark: user_id={user_id}, lat={lat}, lon={lon}, username={username}, password={password}, fakey={fakey}")
    ...
    def attempt_geolocation(self, address):
        log_message(f"Attempting geolocation for address: {address}")
        try:
            if not isinstance(address, str):
                log_message("Invalid address type, address must be a string")
                return None, None
            
            # Use the class attribute for the API key
            geocode_url = f"https://geocode.maps.co/search?q={address}&api_key={self.geocoding_api_key}"

            response = requests.get(geocode_url)
            if response.status_code == 200:
                data = response.json()
                # Move the debugging print statement here
                print(f"API Response Type: {type(data)}, Data: {data}")  # Correct placement of the debugging print statement
                log_message(f"Geolocation API Response: {response.text}")  # For debugging

                # Debugging: Check the type of data received
                log_message(f"Data type received from geolocation API: {type(data)}")

                # Process the response correctly
                if isinstance(data, list) and len(data) > 0:
                    first_result = data[0]
                    print(f"Data type of first_result: {type(first_result)}")  # Debugging print statement
                    if isinstance(first_result, dict):
                        latitude = first_result.get('lat')
                        longitude = first_result.get('lon')
                    else:
                        log_message("Expected a dictionary in the list but got a different type.")
                elif isinstance(data, dict):
                    latitude = data.get('lat')
                    longitude = data.get('lon')
                else:
                    log_message(f"Unexpected data format received from API: {type(data)} - {data}")
                    return None, None

                return latitude, longitude
            else:
                log_message(f"Geocoding API request failed with status {response.status_code}: {response.text}")
                return None, None
        except Exception as e:
            log_message(f"An error occurred during geolocation: {e}")
            return None, None

    def update_adspower_remark(self, user_id, gmail_status, lat, lon, username, password, fakey):
        print(f"Debug Start of update_adspower_remark: user_id={user_id}, lat={lat}, lon={lon}, username={username}, password={password}, fakey={fakey}")
        api_base_url = f"http://{config['adspower']['host']}:{config['adspower']['port']}"
        endpoint = "/api/v1/user/update"
        headers = {'Authorization': config['adspower']['api_key']}
        ...
        accuracy = random.randint(10, 5000) if lat and lon else None

        print(f"Debug before update_data in update_adspower_remark: user_id={user_id}, lat={lat}, lon={lon}, username={username}, password={password}, fakey={fakey}")
        ...
        print(f"Debug after update_data in update_adspower_remark: user_id={user_id}, lat={lat}, lon={lon}, username={username}, password={password}, fakey={fakey}")

         # Debugging: print the update data
        print(f"Update Data: {update_data}")

        # Simulate API call with print statement for debugging purposes
        print(f"Making API call to {api_base_url + endpoint} with data: {update_data}")

        try:
            response = requests.post(api_base_url + endpoint, json=update_data, headers=headers, timeout=10)
            if response.status_code == 200:
                log_message(f"Successfully updated Adspower API for user ID: {user_id}")
            else:
                log_message(f"Failed to update Adspower API for user ID {user_id}: {response.status_code} - {response.text}")
        except requests.exceptions.RequestException as e:
            log_message(f"Exception occurred while updating Adspower API for user ID {user_id}: {e}")
